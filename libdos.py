...
GOLDENEYE_BANNER = ' LibDos by l1b3rtaS <made on GoldenEye>'
...
class LibDos(object):
    ...
    def printHeader(self):
        ...
        # Taunt!
        print()
        print(Fore.YELLOW + GOLDENEYE_BANNER)
        print(Fore.WHITE)
    ...
    def fire(self):
        os.system("clear")
        self.printHeader()
        print(Fore.YELLOW + "Hitting webserver in mode '{0}' with {1} workers running {2} connections each. Hit CTRL+C to cancel.".format(self.method, self.nr_workers, self.nr_sockets))
    ...
    def stats(self):
        ...
        try:
            if self.counter[0] > 0 or self.counter[1] > 0:
                ...
                print(Fore.CYAN + "{0} LibDos strikes hit. ({1} Failed)".format(self.counter[0], self.counter[1]))

                if self.counter[0] > 0 and self.counter[1] > 0 and self.last_counter[0] == self.counter[0] and self.counter[1] > self.last_counter[1]:
                    print(Fore.MAGENTA + "\tServer may be DOWN!")
...
: :: : :  :    :: : ::   :: :  :    : :  :   :: : :   """)
    ...
    print()
    print(Fore.WHITE + ' USAGE: ./libdos.py <url> [OPTIONS]')
    print()
    print(' OPTIONS:')
...
def main():
    ...
    try:
        ...
        libdos = LibDos(url)
        libdos.useragents = useragents
        libdos.nr_workers = workers
        libdos.method = method
        libdos.nr_sockets = socks
